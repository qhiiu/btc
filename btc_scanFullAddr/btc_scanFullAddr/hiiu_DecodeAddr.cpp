...
void hiiu_decodeBase58(std::string address, uint32_t* hash160){

	vector<unsigned char> vectorHash160;
	vectorHash160.clear();

	if (DecodeBase58(address, vectorHash160)) {
		vectorHash160.erase(vectorHash160.begin() + 0);
		vectorHash160.erase(vectorHash160.begin() + 20, vectorHash160.begin() + 24);
	}

	uint8_t _hash160[20];

	for (size_t i = 0; i < vectorHash160.size(); i++) {
		_hash160[i] = vectorHash160.at(i); // --- hash160 đây __hash160[i]
	}
	
	// 20 bytes --> uint32_t _hash160[5]
	memcpy(hash160 , _hash160 , 20);
}
